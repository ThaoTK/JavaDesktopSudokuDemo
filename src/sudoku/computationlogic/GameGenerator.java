...
public class GameGenerator {
     ...
     * - A Random coordinate on the grid is selected. If it is empty, a Random value is is allocated.
     * - The resulting allocation must not produce invalid rows, columms, or squares.
    ...
    private static int[][] getSolvedGame() {
        ...
        for (int value = 1; value <= GRID_BOUNDARY; value++) {
            // allocations refers to the number of times in which a square has been giving a value.
            int allocations = 0;

            // If too many allocation attempts are made which end is an invalid game, we grab the most recent
    ...
    /**
     * The purpose of this function is to take a game which has been already been solved (and thus proven
     * to be solvable), and randomly assign a certain number of tiles to be equal to 0. It appears that there is no
     * straight forward way to check if a puzzle is still solvable after removing the tiles, beyond using
     * another algorithm to attempt to re-solve the problem.
     *
    ...
    private static int[][] unsolveGame(int[][] solvedGame) {
        ...
        while (solvalbe == false) {
            ...
            solvalbe = SudokuSolver.puzzleIsSolvable(toBeSolved);
        }
